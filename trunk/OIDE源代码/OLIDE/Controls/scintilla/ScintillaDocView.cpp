...
void CScintillaView::AddText(char* pchText,long length)
{
	GetCtrl().AddText(length,pchText);
}

void CScintillaView::AddText(wchar_t* pchText,long length)
{
	GetCtrl().AddText(length,pchText);
}

CSyntaxCtrl& CScintillaView::GetCtrl()
{ 
  CSyntaxCtrl& ctrl = m_Edit;
...
CScintillaDoc* CScintillaView::GetDocument() const
{
...
void CScintillaView::Serialize(CArchive& ar)
{
  ...
  if (ar.IsLoading())
  {
		...
		int nDocumentType = GetDocument()->GetDocumentType();

		//create file map
		HANDLE   hMapObject = CreateFileMapping(pFile->m_hFile,NULL,PAGE_READONLY,0,0,NULL);  
		if(hMapObject == NULL)
		{  
			return;  
		}  
		...
		CFileStatus   st;
		pFile->GetStatus(st);
		if(nDocumentType == DT_ANSI)
		{
			rCtrl.AddText(st.m_size,(char*)pBase);
		}
		else if(nDocumentType == DT_UTF8)
		{
			rCtrl.AddUTF8Text(st.m_size, (char*)pBase);
		}
  ...
  else
  {
		...
		char* pchDocument = new char[nDocLength+1];
		TextRange tr;
		tr.chrg.cpMin = 0;
		tr.chrg.cpMax = nDocLength;	
		tr.lpstrText = pchDocument;
		rCtrl.GetTextRange(&tr);
			
		if(nDocumentType == DT_ANSI)
		{
			char* pchANSI;
			int nLength = CScintillaCtrl::UTF82ANSI(pchDocument,nDocLength,pchANSI);
		...
		CFile* pFile = ar.GetFile();
		char* pCurPos = pchDocument;
		int nLength = 0;
		for (int i=0; i<nDocLength; i += 4096)
		{
			nLength = nDocLength - i;
			if(nLength < 4096)
			{
				//Write it to disk
				pFile->Write(pCurPos, nLength);
			}
			else
			{
				//Write it to disk
				pFile->Write(pCurPos, 4096);
				pCurPos += 4096;
			}
		}
...
{
	if((bModified && m_bModified) || (!bModified && !m_bModified))
	{
		return;
	}
...
BOOL CScintillaDoc::DoSave(LPCTSTR pszPathName, BOOL bReplace)
{
	if(IsModified())
	{
		//copy from afx
		...
		if (newName.IsEmpty())
		{
			...
			if (bReplace && newName.IsEmpty())
			{
				...
				//add 
				if (newName.Right(SIZEOF_NO_NULL(STR_MODIFIED_DOCUMENT_TITLE_FLAG))
					== STR_MODIFIED_DOCUMENT_TITLE_FLAG)
				{
					newName.TrimRight(STR_MODIFIED_DOCUMENT_TITLE_FLAG);
				}
	...
	return CDocument::DoSave(pszPathName,bReplace);
}

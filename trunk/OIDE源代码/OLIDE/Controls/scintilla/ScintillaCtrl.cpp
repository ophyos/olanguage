...
int CScintillaCtrl::WCS2MBS( const wchar_t* pwcs, int nLength,char*& psz)
{   
	int lengthOfMbs = WideCharToMultiByte( CP_ACP, 0, pwcs, nLength, NULL, 0, NULL, NULL);

	if(lengthOfMbs == 0)
		lengthOfMbs = 1;

	psz = new char[ lengthOfMbs];
	psz[0] = '\0';

	int nCharsWritten = WideCharToMultiByte( CP_ACP, 0, pwcs, nLength, psz, lengthOfMbs, NULL, NULL);	

	if (nLength != -1)
		return nCharsWritten; //Trust the return value
	else
		return nCharsWritten ? nCharsWritten - 1 : 0;
}

int CScintillaCtrl::MBS2WCS( const char* pmbs, int nLength,wchar_t*& pwcs) 
{   
	int lengthOfWcs = MultiByteToWideChar( CP_ACP, 0, pmbs, nLength, NULL, 0 );

	if(lengthOfWcs == 0)
		lengthOfWcs = 1;

	pwcs = new wchar_t[ lengthOfWcs];
	pwcs[0] = L'\0';

	int nCharsWritten = MultiByteToWideChar( CP_ACP, 0, pmbs, nLength, pwcs, lengthOfWcs );
...
int CScintillaCtrl::ANSI2UTF8(const char* pszText,int nLength,char*& pszUTF8)
{
	wchar_t* pwchar;
	int lengthMbs = MBS2WCS(pszText,nLength,pwchar);
	if(nLength == -1)
	{
		++lengthMbs;
	}
	...
	int lengthUTF8 = W2UTF8(pwchar, lengthMbs, pszUTF8);
...
int CScintillaCtrl::UTF82ANSI(const char* pszUTF8Text,int nLength,char*& pchANSI)
{
	//Convert the UTF8 text to Unicode
	wchar_t* pszWText;
	int lengthWcs = UTF82W(pszUTF8Text, nLength, pszWText);
	if(nLength == -1)
	{
		++lengthWcs;
	}
	
	int lengthANSI = WCS2MBS(pszWText,lengthWcs,pchANSI);
...
void CScintillaCtrl::AddText(int length, const wchar_t* text, BOOL bDirect)
{
  ...
  AddUTF8Text(nUTF8Length, pszUTF8, bDirect);
...
{
	char* putf8Text;
	int nLength = CScintillaCtrl::ANSI2UTF8(text,length,putf8Text);

	AddUTF8Text(nLength, putf8Text,bDirect);
...
CString CScintillaCtrl::GetCurLine()
{
	int len=Call(SCI_GETCURLINE, 0, 0, TRUE)+1;
	CString s;
	TCHAR* pszSelText = s.GetBufferSetLength(len);
	Call(SCI_GETCURLINE, len, reinterpret_cast<LPARAM>(pszSelText), TRUE);
	s.ReleaseBuffer(len);	
	return s;
}
...
void CScintillaCtrl::AutoCShow(int lenEntered, const wchar_t* itemList, BOOL bShow, BOOL bDirect)
{
  //Convert the unicode text to UTF8
  char* pszUTF8;
  ...
  //Call the native scintilla version of the function with the UTF8 text
  AutoCShow(lenEntered, pszUTF8, bShow,bDirect);
...
CString CScintillaCtrl::GetLine( int lineNum )
{
	int len=Call(SCI_GETLINE, lineNum, 0, TRUE)+1;
	CString s;
	TCHAR* pszSelText = s.GetBufferSetLength(len);
	Call(SCI_GETLINE, lineNum, reinterpret_cast<LPARAM>(pszSelText), TRUE);
	s.ReleaseBuffer(len);	
	return s;
}
...
void CScintillaCtrl::AddUTF8Text(int length, const char* text, BOOL bDirect)
{
...
void CScintillaCtrl::AutoCShow(int lenEntered, const char* itemList, BOOL bShow, BOOL bDirect)
{
	if(bShow)
	{
	...
	{
		Call(SCI_AUTOCSHOWEX, static_cast<WPARAM>(lenEntered), reinterpret_cast<LPARAM>(itemList), bDirect);
	}  
}
...
void CScintillaCtrl::AutoCSelect(int nItem, BOOL bDirect)
{
	Call(SCI_AUTOCSELECT, nItem, 0, bDirect);
}

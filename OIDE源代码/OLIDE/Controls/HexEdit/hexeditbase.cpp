...
{	
	...
	MoveCaretPos(cCarretPoint);
	ShowCaret();
}

void CHexEditBase::MoveCaretPos(POINT point)
{
	SetCaretPos(point);
}
...
{	
	...
	// menue-item: copy
	cMenu.AppendMenu(IsSelection() ? MF_STRING : MF_GRAYED|MF_DISABLED|MF_STRING, ID_EDIT_COPY, (LPCTSTR)m_cContextCopy);
	...
	cMenu.AppendMenu(cSource.IsDataAvailable(m_nBinDataClipboardFormat) && !m_bReadOnly ? MF_STRING : MF_GRAYED|MF_DISABLED|MF_STRING, 
		ID_EDIT_PASTE, (LPCTSTR)m_cContextPaste);
	cSource.Release();
	...
	if( (m_nSelectionBegin != NOSECTION_VAL) && (m_nSelectionEnd != NOSECTION_VAL) ) {
		...
		try {
			...
			*pData = nLength;
			...
			memcpy(pPtr, (LPCTSTR)cStr, cStr.GetLength()*sizeof(TCHAR));
			::GlobalUnlock(hMema);
...
UINT CHexEditBase::GetData(BYTE *pByte, UINT nLength,UINT nOffset)
{
    if(m_pData != NULL) {
		memcpy(pByte, m_pData+nOffset, min(nLength, m_nLength-nOffset));
		return m_nLength;
	} else {
		return 0;
	}
